...
const set_amounts = (controller_name, form) => {
  const url = Routes[controller_name + '_amounts_path']() + '?' + form.serialize();
  $.get(url, () => {
    return;
  });
};
...
const get_amounts = (controller_name, form, callback) => {
  const url = Routes[controller_name + '_amounts_path']() + '?' + form.serialize();
  $.ajax({
    url: url,
    dataType: 'json',
...
const get_item_amount = (quantity, unitary_cost, discount, callback) => {
  const url = Routes['items_amount_path']() +
...
const get_defaults_taxes = (callback) => {
  const url = Routes['taxes_get_defaults_path']();
  $.ajax({
    url: url,
    dataType: 'json',
    success: (data) => {
...
const get_id_prefix = (input_field) => {
  let id_prefix = input_field.attr('id');
...
const init_invoice_item_autocomplete = (input_field) => {
  const id_prefix = get_id_prefix(input_field);
...
const destroy_invoice_item_autocomplete = (input_field) => {
...
jQuery(document).ready(($) => {
  ...
  $('[data-role="add-item"]').each(function() {
    const self = $(this);

    const insertionNode = self.data("insertion-node");
    const eventData = {
  ...
  $('body').on("click", '[data-role="remove-item"]', function(e) {
    e.preventDefault();
    const self = $(this);
  ...
  const form = $('form[data-role="invoice"]');
  const controller_name = form.data('controller');  // REQUIRED!!!

  // DOM caching
  const invoice_series = $('#invoice_series_id');
  ...
  form.find('[data-changes="amount"]')
    ...
    .on('change input', '.js-item', (e) => {
      ...
      const item_row = item.parents('.js-item');
      const quantity = item_row.find('[data-role="quantity"]').val();
      const price = item_row.find('[data-role="unitary-cost"]').val();
      const discount = item_row.find('[data-role="discount"]').val();
      get_item_amount(quantity, price, discount, (data) => {
        const net_amount = data.amount;
  ...
  form.find('[data-role="item-description"]').each(function() {
  ...
  form.on('cocoon:after-insert', (e, item) => {
    if (item.hasClass('js-payment')) {
      ...
      const amount_item = item.find('input[name*=amount]');
      get_amounts(controller_name, form, (data) => {
        let amount = data.gross_amount - data.paid_amount;
        amount = amount > 0 ? amount : 0;
        amount_item.val(Math.round(amount * 10 ** data.precision) / 10 ** data.precision);
        return;
      });
      // default date is today
      const date_item = item.find('input[name*=date]');
      ...
      const tax_selector = item.find('[data-role="taxes-selector"]');
      tax_selector.trigger('update');
      // Set default tax values
      get_defaults_taxes((data) => {
        const new_val = data.data.map(tax => tax.id);
  ...
  form.on('click', '[data-role="taxes-selector-label"]', function(e) {
  ...
  form
    .on('update', '[data-role="taxes-selector"]', function() {
      const checked_taxes = $(this).find(':checked');
      const label = [];
      checked_taxes.each(function() {
        label.push($(this).closest('.checkbox').text().trim());
      });
      const total = label.length > 1 ? ('(' + label.length + ')') : '';
      const labelText = label.length > 0 ? label.join(', ') : 'None';
      $(this).find('[data-role="label"]').html(labelText);
      $(this).find('[data-role="total"]').html(total);
    })
    .on('change', '[data-role="taxes-selector"] :checkbox', function(e) {
  ...
  form.on('select2:select select2:unselect', '[data-role="taxes-selector"]', function() {
    const parent_form = $(this).closest('form[data-role="invoice"]');
    const controller = parent_form.data('controller');
  ...
  $(document).on('click', '[data-role="csv-form"]', (e) => {
    e.preventDefault();
    const old_action = $('#js-search-form').attr('action');
    // To download csv you must split by the querystring
    const new_action = old_action.split("?")[0];
  ...
  // Submit Form
  $(document).on('click', '[data-role="submit-form"]', function(e) {
    e.preventDefault();
    ...
    if ($(this).data('activateFlag')) {
      const flagName = $(this).data('activateFlag');
  ...
  const sectionInfo = $('#js-section-info');
  const sectionInfoButton = $('#js-section-info-button');
